...
macro_rules! implement_binary_tree_attributes {
    ($struct:ident) => {
        ...
        impl<T: Copy + std::cmp::PartialEq + std::fmt::Display> $struct<T> {
            pub fn create(
                data: T,
                left: Option<Box<$struct<T>>>,
                right: Option<Box<$struct<T>>>,
            ) -> $struct<T> {
                Self {
                    _data: data,
                    _left: left,
            ...
            pub fn get_left_child(&self) -> Option<&Box<$struct<T>>> {
                self._left.as_ref()
            }

            pub fn get_right_child(&self) -> Option<&Box<$struct<T>>> {
                self._right.as_ref()
            }
...
pub(crate) use implement_binary_tree_attributes;

implement_binary_tree_attributes!(Node);

impl<T: Copy + std::cmp::PartialEq + std::fmt::Display> Node<T> {
